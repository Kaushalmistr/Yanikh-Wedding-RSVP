...
-- Run this if the schema check shows missing columns
-- ============================================

-- STEP 1: Drop existing tables (if they exist with wrong schema)
DROP TABLE IF EXISTS uploaded_guest_lists CASCADE;
...
-- STEP 4: Create Wedding Events table
CREATE TABLE wedding_events (
  ...
  description TEXT,
  cover_image TEXT,
  created_by UUID REFERENCES users(id),  -- NULL allowed (no NOT NULL constraint)
  created_at TIMESTAMPTZ DEFAULT NOW()
);

-- STEP 5: Create Guests table WITH ALL COLUMNS
CREATE TABLE guests (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
-- STEP 6: Create Bulk Messages table
...
-- STEP 7: Create Uploaded Guest Lists table
CREATE TABLE uploaded_guest_lists (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  event_id UUID NOT NULL REFERENCES wedding_events(id) ON DELETE CASCADE,
...
CREATE INDEX idx_wedding_events_rsvp_token ON wedding_events(rsvp_token);
CREATE INDEX idx_guests_mobile ON guests(mobile);
CREATE INDEX idx_bulk_messages_event_id ON bulk_messages(event_id);
CREATE INDEX idx_uploaded_guest_lists_event_id ON uploaded_guest_lists(event_id);
